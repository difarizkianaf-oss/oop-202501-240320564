...
public class ShoppingCart {
    ...
    public void addProduct(Product p, int qty) throws InvalidQuantityException, NegativePriceException {
        if (p.getPrice() < 0) {
            throw new NegativePriceException("Harga tidak boleh negatif: " + p.getName());
        }
        if (qty <= 0) {
